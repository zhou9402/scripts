...
# ============== Fused GDN Kernel (with Delta output for debugging) ==============
@ct.kernel
def fused_gdn_kernel(
    ...
    # Causal mask (lower triangular, using ct.where for precision)
    offs_row = ct.arange(chunk_size, dtype=ct.int32)[:, None]
    offs_col = ct.arange(chunk_size, dtype=ct.int32)[None, :]
    causal_mask = offs_row >= offs_col  # [chunk_size, chunk_size] bool
    ...
    for c in range(num_chunks):
        ...
        qk_gated = qk * g_attn  # Apply gate
        qk_masked = ct.where(causal_mask, qk_gated, 0.0)  # Apply causal mask with ct.where (no precision loss)
...
def fused_gdn(S, T, G, Beta, V, Q, K, chunk_size=CHUNK_SIZE):
    """Fused GDN forward pass. Returns O, Delta for debugging.
    
    Note: Q and K should be L2 normalized before calling this function.
    """
    ...
    with torch.cuda.device(K.device):
        ct.launch(torch.cuda.current_stream(), (B_dim * H,), fused_gdn_kernel,
                  (S, T_r, V_r, Q_r, K_r, G_r, Beta_r, O_r, Delta_r, d, chunk_size, H))
    
    return (O_r.reshape(B_dim, seq_len, H, d), Delta_r.reshape(B_dim, seq_len, H, d))
...
def compare_tensors(name, t1, t2, atol=1e-2, rtol=1e-2, show_values=True):
    ...
    # |diff| < 2e-4
    num_close_4 = (diff < 2e-4).sum().item()
    pct_close_4 = 100.0 * num_close_4 / total
    
    status = f"{GREEN}✓{RESET}" if is_close else f"{RED}✗{RESET}"
    print(f"  {status} {name}: max={max_diff:.6f}, mean={mean_diff:.6f}")
    print(f"      |diff|<1e-3: {num_close_3}/{total} ({pct_close_3:.2f}%), |diff|<2e-4: {num_close_4}/{total} ({pct_close_4:.2f}%)")
...
def test_fused_gdn(batch_size=1, seq_len=128, num_heads=2, head_dim=128, chunk_size=64, atol=1e-2, rtol=1e-2):
    ...
    # Generate raw Q, K, V (L2 norm will be applied before calling kernels)
    ...
    Beta = torch.rand(batch_size, seq_len, num_heads, dtype=torch.float32, device=device).sigmoid().to(dtype)
    G_raw = F.logsigmoid(torch.rand(batch_size, seq_len, num_heads, dtype=torch.float32, device=device))
    G_raw = G_raw / gate_logit_normalizer
    ...
    # L2 normalize Q and K before passing to both FLA and GDN
    ...
    A = chunk_scaled_dot_kkt_fwd(k=K_norm, beta=Beta, g_cumsum=G_cumsum, cu_seqlens=None, chunk_size=chunk_size, output_dtype=torch.float32)
    A_solved = solve_tril(A=A, cu_seqlens=None, output_dtype=K.dtype)
    w_fla, u_fla = recompute_w_u_fwd(k=K_norm, v=V, beta=Beta, A=A_solved, g_cumsum=G_cumsum, cu_seqlens=None)
    
    S_fla = S_init.clone()
    h_fla, v_new_fla = chunk_gated_delta_rule_fwd_h(k=K_norm, w=w_fla, u=u_fla, g=G_cumsum, initial_state=S_fla,
                                                     initial_state_indices=initial_state_indices, cu_seqlens=None)
    O_fla = chunk_fwd_o(q=Q_norm, k=K_norm, v=v_new_fla, h=h_fla, g=G_cumsum, scale=scale, cu_seqlens=None)
    ...
    # Q and K are already L2 normalized
    O_gdn, Delta_gdn = fused_gdn(S=S_gdn, T=A_solved, G=G_cumsum, Beta=Beta, V=V, Q=Q_norm, K=K_norm, 
                                  chunk_size=chunk_size)
    
    # ========== Compare Delta (v_new) ==========
    print(f"\n{BOLD}Step 1: Compare Delta (v_new):{RESET}")
    # v_new_fla is the delta that FLA passes to chunk_fwd_o
    delta_ok = compare_tensors("Delta (v_new)", v_new_fla, Delta_gdn, atol, rtol)
    
    # ========== Compare O and S ==========
    print(f"\n{BOLD}Step 2: Compare O and S:{RESET}")
    ...
    # Print 100 random slices of S for comparison
    print(f"\n{BOLD}Random S slices (100 samples):{RESET}")
    S_fla_flat = S_fla.flatten()
    S_gdn_flat = S_gdn_as_h.flatten()
    total_elements = S_fla_flat.numel()
    torch.manual_seed(123)  # For reproducibility
    random_indices = torch.randperm(total_elements)[:100]
    print(f"  {'idx':>8} | {'Triton (FLA)':>14} | {'cuda.tile (GDN)':>14} | {'diff':>12}")
    print(f"  {'-'*8}-+-{'-'*14}-+-{'-'*14}-+-{'-'*12}")
    for idx in random_indices:
        fla_val = S_fla_flat[idx].item()
        gdn_val = S_gdn_flat[idx].item()
        diff = abs(fla_val - gdn_val)
        print(f"  {idx.item():>8} | {fla_val:>14.6f} | {gdn_val:>14.6f} | {diff:>12.2e}")
    
    all_ok = delta_ok and o_ok and s_ok
    print()
...
if __name__ == "__main__":
    ...
    test_fused_gdn(args.batch_size, args.seq_len, args.num_heads, args.head_dim, args.chunk_size, 
                   args.atol, args.rtol)
